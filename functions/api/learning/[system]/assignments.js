...
export async function onRequestGet({ request, env, params }) {
  ...
  try {
    ...
    const { results } = await env.DB.prepare(
      'SELECT a.id, a.title, c.name AS class_name, a.question_ids, a.due_at, s.score, s.submitted_at ' +
      ...
      'WHERE c.system_code = ? ORDER BY a.created_at DESC'
    ).bind(user.id, user.id, system).all();

    const items = results.map((row) => {
      let questionCount = 0;
      try { const arr = JSON.parse(row.question_ids || '[]'); questionCount = Array.isArray(arr) ? arr.length : 0; } catch {}
      return {
        id: row.id, title: row.title, class_name: row.class_name,
        question_count: questionCount, due_at: row.due_at,
