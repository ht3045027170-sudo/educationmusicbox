(() => {
  ...
  tab.id = 'teacherWorkbenchTab'; tab.dataset.tab = 'questions'; tab.textContent = '出题工作台';
  ...
  section.innerHTML = `
    <header class="tw-hero">
      <div><span>HAITANG EXAM · AUTHORING DESK</span><h1>教师出题工作台</h1><p>制作音乐试题、试听谱面，并实时核对学生最终看到的内容。</p></div>
    ...
    <div class="tw-actions">
      ...
      <button type="button" disabled><b>⚡</b><span>快速出题<small>第二阶段开放</small></span></button>
      <button type="button" disabled><b>▤</b><span>新建试卷<small>第三阶段开放</small></span></button>
      <button type="button" disabled><b>AI</b><span>AI 辅助出题<small>审核流程成熟后开放</small></span></button>
    </div>
    ...
    <section class="tw-library">
      <div class="tw-library-head"><div><span>MY QUESTION BANK</span><h2>我的题库</h2></div><div class="tw-filters"><select id="twSubject"><option value="">全部科目</option><option value="dictation">听记</option><option value="theory">乐理</option><option value="sight_singing">视唱</option></select><select id="twStatus"><option value="">全部状态</option><option value="draft">草稿</option><option value="submitted">待审</option><option value="published">已发布</option><option value="changes_requested">需修改</option></select><input id="twSearch" placeholder="搜索题干或知识点"><button type="button" id="twSearchButton">查询</button></div></div>
  ...
  style.textContent = `
    ...
    .tw-library{margin-top:24px;padding:22px;border:1px solid var(--tw-line);border-radius:18px;background:rgba(255,253,249,.85)}.tw-library-head{display:flex;align-items:end;justify-content:space-between;gap:20px}.tw-library h2{margin:5px 0 0;font:500 30px Georgia,"Songti SC",serif}.tw-filters{display:flex;gap:8px;flex-wrap:wrap}.tw-filters input,.tw-filters select,.tw-settings input,.tw-settings select,.tw-compose input,.tw-compose select,.tw-compose textarea{border:1px solid var(--tw-line);border-radius:10px;background:#fff;padding:10px 11px;color:var(--tw-ink);font:inherit}.tw-question-list{display:grid;gap:9px;margin-top:18px}.tw-question{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:16px;align-items:center;padding:15px 16px;border:1px solid #eadfd5;border-radius:13px;background:#fff}.tw-question h3{margin:0 0 6px;font-size:16px}.tw-question p{margin:0;color:var(--tw-muted);font-size:12px}.tw-question aside{display:flex;align-items:center;gap:10px}.tw-status{padding:5px 9px;border-radius:999px;background:var(--tw-wash);font-size:12px;color:var(--tw-ink)}.tw-empty{padding:30px;text-align:center;color:var(--tw-muted)}
    .tw-type-dialog{width:min(1000px,calc(100% - 28px));max-height:calc(100dvh - 28px);padding:0;border:0;border-radius:22px;background:#fbf7f1;color:var(--tw-ink)}.tw-type-dialog::backdrop,.tw-editor::backdrop{background:rgba(43,27,20,.64);backdrop-filter:blur(8px)}.tw-type-head{display:flex;justify-content:space-between;padding:28px 30px 20px;border-bottom:1px solid var(--tw-line)}.tw-type-head h2{margin:7px 0;font:500 38px Georgia,"Songti SC",serif}.tw-type-head p{margin:0;color:var(--tw-muted)}.tw-type-head [data-close]{align-self:start;background:transparent;color:var(--tw-ink);font-size:28px;padding:0}.tw-type-groups{padding:12px 30px 30px;overflow:auto}.tw-type-groups h3{margin:20px 0 10px;font-size:13px;color:var(--tw-muted)}.tw-type-groups section>div{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}.tw-type-groups [data-type]{display:grid;justify-items:start;gap:6px;min-height:126px;padding:17px;border:1px solid var(--tw-line);border-radius:15px;background:#fff;color:var(--tw-ink);text-align:left}.tw-type-groups [data-type]:hover{border-color:var(--tw-accent);transform:translateY(-2px)}.tw-type-groups i{font:normal 28px Georgia,serif;color:var(--tw-accent)}.tw-type-groups span{color:var(--tw-muted);font-size:12px}
    .tw-editor{width:calc(100% - 24px);height:calc(100dvh - 24px);max-width:none;max-height:none;padding:0;border:0;border-radius:20px;background:#eee5db;color:var(--tw-ink);overflow:hidden}.tw-editor form{height:100%;display:flex;flex-direction:column}.tw-editor-head{display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:18px;min-height:74px;padding:10px 18px;background:#fffdf9;border-bottom:1px solid var(--tw-line)}.tw-editor-head>button{justify-self:start;background:transparent;color:var(--tw-ink);padding-left:0}.tw-editor-head>div:nth-child(2){text-align:center}.tw-editor-head h2{margin:3px 0 0;font:500 22px Georgia,"Songti SC",serif}.tw-save-cluster{justify-self:end;display:flex;align-items:center;gap:9px}.tw-save-cluster>span{font-size:12px;color:var(--tw-muted)}.tw-editor-grid{display:grid;grid-template-columns:250px minmax(480px,1fr) minmax(280px,360px);flex:1;min-height:0}.tw-settings,.tw-compose,.tw-preview{min-height:0;overflow:auto;padding:22px}.tw-settings{background:#f4ede4;border-right:1px solid var(--tw-line)}.tw-settings h3,.tw-compose h3,.tw-preview h3{margin:7px 0 18px;font:500 25px Georgia,"Songti SC",serif}.tw-settings>label{display:grid;gap:6px;margin-top:14px;font-size:13px}.tw-settings label small{color:var(--tw-muted)}.tw-type-rail{display:grid;gap:6px}.tw-type-rail button{display:flex;align-items:center;gap:8px;width:100%;padding:9px 11px;border-radius:9px;background:transparent;color:var(--tw-ink);text-align:left}.tw-type-rail button.active{background:var(--tw-ink);color:#fff}.tw-compose{background:#f9f5ef}.tw-field,.tw-answer-grid label{display:grid;gap:7px;font-size:13px}.tw-specific{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:14px 0}.tw-specific label{display:grid;gap:6px;font-size:12px}.tw-paper{max-width:860px;margin:18px auto;padding:16px;border-radius:4px;background:var(--tw-paper);box-shadow:0 15px 38px rgba(77,48,32,.13)}.tw-paper-label{display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;color:var(--tw-muted);font-size:12px}.tw-paper-label button{padding:8px 14px}.tw-answer-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:16px 0}.tw-preview{background:#eee5db;border-left:1px solid var(--tw-line)}.tw-student-card{min-height:420px;padding:24px 20px;border-radius:16px;background:#fff;box-shadow:0 10px 30px rgba(79,49,34,.1)}.tw-student-card .student-count{font:700 11px ui-monospace,monospace;letter-spacing:.12em;color:var(--tw-muted)}.tw-student-card h4{margin:24px 0 18px;font:500 21px/1.5 Georgia,"Songti SC",serif}.tw-student-score{margin:12px -8px;padding:8px;border-radius:10px;background:#fffdf8}.tw-student-options{display:grid;gap:8px}.tw-student-options div{padding:11px 12px;border:1px solid #e5d9ce;border-radius:10px}.tw-student-meta{display:flex;justify-content:space-between;margin-top:16px;color:var(--tw-muted);font-size:12px}.tw-student-play{width:100%;margin-top:8px}.tw-mobile-tabs{display:none}
  ...
  let csrfToken = '', current = null, currentType = 'interval', dirty = false, loadingEditor = false;
  ...
  async function loadBank(){
    const query=new URLSearchParams({page:1,pageSize:50,systemCode:'gaokao',subject:document.getElementById('twSubject').value,status:document.getElementById('twStatus').value,search:document.getElementById('twSearch').value});
    ...
    document.getElementById('twTotal').textContent=data.pagination.total;
    ...
    document.getElementById('twQuestionList').innerHTML=data.items.map(q=>{const type=TYPES[inferType(q)]||TYPES.theory_choice;return `<article class="tw-question"><div><h3>${type.icon} ${esc(q.content.prompt||'未命名题目')}</h3><p>${esc(type.label)} · ${esc(q.knowledge_id)} · 难度 ${q.difficulty} · v${q.version_no}</p></div><aside><span class="tw-status">${STATUS[q.status]||esc(q.status)}</span><button type="button" data-edit="${q.id}">继续编辑</button></aside></article>`}).join('')||'<p class="tw-empty">没有符合条件的题目。点击“新建试题”开始制作。</p>';
  }
  ...
  function specificHTML(type){
    ...
    if(type==='interval') return `<label>根音<select data-param="root">${NOTE_OPTIONS}</select></label><label>音程<select data-param="interval">${INTERVALS.map(([n,s])=>`<option value="${s}">${n}</option>`).join('')}</select></label><label>播放方式<select data-param="playMode"><option value="melodic">旋律音程</option><option value="harmonic">和声音程</option></select></label><label>方向<select data-param="direction"><option value="up">上行</option><option value="down">下行</option></select></label><label><span>&nbsp;</span><button type="button" data-distractors>自动生成干扰项</button></label>`;
  ...
  section.onclick=event=>{if(event.target.closest('[data-new]'))typeDialog.showModal();if(event.target.closest('[data-refresh]'))loadBank();const edit=event.target.closest('[data-edit]');if(edit)openById(edit.dataset.edit)};
  ...
  document.getElementById('twSearchButton').onclick=loadBank; document.getElementById('twSubject').onchange=loadBank; document.getElementById('twStatus').onchange=loadBank;
  ...
  editor.querySelector('[data-close]').onclick=()=>{if(dirty&&!confirm('还有未保存更改，确定返回题库吗？'))return;window.MusicEditor?.destroy?.();editor.close()};
