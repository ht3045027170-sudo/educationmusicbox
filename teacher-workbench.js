(() => {
  ...
  section.onclick=event=>{if(event.target.closest('[data-new]'))typeDialog.showModal();if(event.target.closest('[data-quick]'))quickDialog.showModal();if(event.target.closest('[data-new-paper]')){document.querySelector('[data-tab="teaching"]')?.click();setTimeout(()=>document.querySelector('.set-builder')?.scrollIntoView({behavior:'smooth',block:'start'}),100)}if(event.target.closest('[data-refresh]'))loadBank();const edit=event.target.closest('[data-edit]');if(edit)openById(edit.dataset.edit)};
