(() => {
  ...
  async function downscaleToDataUrl(file, maxSide = 1800) {
    ...
    const blob = await new Promise((resolve, reject) => canvas.toBlob(
      value => value ? resolve(value) : reject(new Error('图片处理失败，请换一张或重启应用再试')),
      type, .92
    ));
    return await new Promise((resolve, reject) => {
      const reader = new FileReader();
      reader.onload = () => resolve({ dataUrl: reader.result, width: targetWidth, height: targetHeight });
  ...
  async function importImage(file) {
    ...
    try {
      showStatus('sightScanStatus', '正在处理图片，请稍候…');
      ...
      setText('sightFileMeta', `${file.name} · ${width} × ${height}`);
      showStep('prepare', true);
      renderSource();
      showStatus('sightScanStatus', '图片已优化导入，可调整对比度与去灰度。');
      await persist(false);
    } catch (error) {
      showStatus('sightScanStatus', `无法导入图片：${error.message}`, true);
  ...
  let referenceImageTimer = 0;
  let referenceImageToken = 0;
  function updateReferenceImageSoon() {
    if (referenceImageTimer) return;
    referenceImageTimer = setTimeout(() => {
      referenceImageTimer = 0;
      const canvas = $('sightSourceCanvas');
      const image = $('sightReferenceImage');
      if (!canvas?.width || !image) return;
      const token = ++referenceImageToken;
      canvas.toBlob(blob => {
        if (token !== referenceImageToken || !blob) return;
  ...
  let sliderRenderQueued = false;
  function queueSliderRender() {
    if (sliderRenderQueued) return;
    sliderRenderQueued = true;
    requestAnimationFrame(() => { sliderRenderQueued = false; renderSource(); });
  }
  $('sightContrast').addEventListener('input', event => { project.preprocessing.contrast = Number(event.target.value); queueSliderRender(); });
  $('sightThreshold').addEventListener('input', event => { project.preprocessing.threshold = Number(event.target.value); queueSliderRender(); });
